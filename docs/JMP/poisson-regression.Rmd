...
&emsp;When we are looking to determine which factor(s) are associated with some response variable that is a count of some occurrence (such as the number of birds that visit a bird feeder over an hour or the number of ticks found on bear cubs), we can use Poisson regression, a special type of Generalized Linear Regression.  
