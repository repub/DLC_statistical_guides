...
&emsp;While larger contingency tables (2x3, 2x4, etc.) and sample sizes can be used with the Fisher's exact test, as either increases so does the computation time.  This is because unlike some other statistical tests, Fisher's exact test calculates the exact p-value.  Therefore, for larger sample sizes (>1000) or when the expected counts are > 5 for at least 80% of the cells then a [&chi;<sup>2</sup> test](https://tylerbg.github.io/DLC_statical_guides/docs/Info/chi-sq-test) may be preferred.
