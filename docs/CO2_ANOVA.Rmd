...
$$\mathbfit{y_i = {\beta}_0 + {\beta}_1 x_{i,1} + {\beta}_2 x_{i,2} + ... + {\beta}_n x_{i,n} + {\epsilon}_i}$$

- $\mathbfit{y_i}$ is the response variable
- $\mathbfit{x_i}$ are our predictor variables
- $\mathbfit{\beta}$ are coefficients for the predictor variables and the intercept
- $\mathbfit{{\epsilon}_i}$ is the error term

In our case, our equation will look like:

$$\mathbfit{y_{i, Uptake} = {\beta}_0 + {\beta}_{Treatment} x_{i,Treatment} + {\beta}_{Type} x_{i,Type} + {\beta}_{conc} x_{i,conc} + {\epsilon}_i}$$
...
$$\mathbfit{y_{i, Uptake} = 29.260 + 0.018 x_{i,Treatment} - 6.860 x_{i,Type} - 12.660 x_{i,conc}}$$
...
Notice that the Type and Treatment variables are concatenated with "Mississippi" and "chilled", respectively.  When Type is "Quebec", the dummy code is 0 and therefor the value will be 0 in the regression equation ($\mathbfit{\beta_3 * x_{i,Type} = -12.659 * 0 = 0}$).  Conversely, when Type is "Mississippi" which is dummy coded to 1 the value for that variable will equal the estimated coefficient ($\mathbfit{\beta_3 * x_{i,Type}=-12.659 * 1 = -12.659}$).  If we happened to have a third level in Type, the value for that factor would be double the estimated coefficient, triple for a fourth level, and so forth.
