...
glht(iris_mlr, mcp(Species = "Tukey"))
...
```{r}
library(tidyverse)
cars_df <- read.csv("../../dat-source/car.data", header = FALSE)

head(cars_df)

colnames(cars_df) <- c("Buying Price", "Maintenance Price", "Doors", "Capacity", "Luggage Size", "Safety","Acceptability")

cars_df <- cars_df %>%
  select(-c(`Maintenance Price`:`Luggage Size`))

cars_df %>%
  group_by_all() %>%
  summarize(n = n()) %>%
  pivot_wider(names_from = Acceptability,
              values_from = n,
              values_fill = 0)
...
```{r}
detergent <- read.csv("../../dat-source/detergent.csv")

head(detergent)

detergent_wide <- detergent %>%
  pivot_wider(names_from = Brand,
              values_from = Count,
              values_fill = 0)

detergent_mlr <- multinom(cbind(x, m, c) ~ ., data = detergent_wide)
...
```{r}
df <- read.csv("http://peopleanalytics-regression-book.org/data/health_insurance.csv")

df <- df %>%
  tibble() %>%
  filter(gender != "Non-binary") %>%
  select(!c(position_level, household)) %>%
...
# 
df %>%
  write_csv("../../dat/health_insurance.csv")
...
```{r}
ggplot(health, aes(x = plan, y = age)) +
  geom_boxplot()

ggplot(health, aes(x = plan, fill = gender)) +
