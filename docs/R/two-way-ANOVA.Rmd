...
&emsp;The physiologist wants to test the following questions and their respective null and alternative hypotheses:
...
&emsp;Using the `summary()` (or `anova()`) command we can print a table of results from our fitted ANOVA<sup>2</sup> model.
...
&emsp;Since the interaction term is insignificant we should not follow up on that part of the ANOVA model with post-hoc tests.  So, to leave out a lot of unnecessary output we can first assign the results from the Tukey post-hoc test from the `TukeyHSD()` function to an object and then use `$` to only print the pairwise comparisons from *Smoking History* and *Test*.
